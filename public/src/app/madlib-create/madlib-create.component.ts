	...
	validateWords(newStr="") {
		return new Promise(resolve => {
			...
			for (let i = 0; i < this.words_field.controls.length; i++) {
				...
				this._httpService.validateWord(this.words_field.value[i])
					.subscribe(data => {
						console.log(data)
						...
						if (data.results[1].partOfSpeech == this.madlib.blanks[i]) {
							console.log(`Word type: ${data.results[1].partOfSpeech}, matches: ${this.madlib.blanks[i]}`)
